    ...
    return (
      <div className="space-y-8">
        ...
        <div className="p-4 bg-destructive/15 text-destructive rounded-md">
          Você não tem permissão para acessar esta página. Apenas médicos e enfermeiros podem visualizar prontuários.
        </div>
      </div>
    )
  }
              ...
              {prontuarios.length === 0 ? (
                ...
                prontuarios.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-6 py-4">{new Date(item.ultima_atualizacao).toLocaleString('pt-BR')}</td>
